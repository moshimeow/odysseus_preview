...
/// Relative pose constraint with visually-informed covariance
///
/// This encodes the relative motion between two frames as learned from VIO optimization.
/// The covariance is tighter than pure IMU preintegration because it incorporates
/// information from visual constraints.
///
/// Mode A implementation: Simple 9-DOF constraint without bias handling.
/// GBA uses LBA's bias estimates directly or handles biases separately.
#[derive(Debug, Clone)]
pub struct RelativePoseConstraint {
    /// Relative rotation (R_i^T * R_j) as quaternion
    pub delta_rotation: Quat<f64>,
    /// Relative velocity in frame i: R_i^T * (v_j - v_i - g*dt)
    pub delta_velocity: Vector3<f64>,
    /// Relative position in frame i: R_i^T * (p_j - p_i - v_i*dt - 0.5*g*dt²)
    pub delta_position: Vector3<f64>,
    /// 9x9 posterior covariance (tighter than IMU-only due to visual constraints)
    pub covariance: Matrix9<f64>,
    /// Time delta between frames
    pub delta_time: f64,
    /// Frame index of the first frame
    pub frame_i: usize,
    /// Frame index of the second frame
    pub frame_j: usize,
}

impl RelativePoseConstraint {
    /// Create from optimized VIO poses and extracted covariance
    ///
    /// # Arguments
    /// * `pose_i` - Optimized pose of frame i (rotation as quat, translation)
    /// * `velocity_i` - Optimized velocity of frame i (world frame)
    /// * `pose_j` - Optimized pose of frame j
    /// * `velocity_j` - Optimized velocity of frame j
    /// * `gravity` - Gravity vector in world frame
    /// * `delta_time` - Time between frames
    /// * `covariance` - 9x9 posterior covariance extracted from VIO Hessian
    /// * `frame_i`, `frame_j` - Frame indices
    pub fn from_optimized_poses(
        rotation_i: &Quat<f64>,
        translation_i: &Vector3<f64>,
        velocity_i: &Vector3<f64>,
        rotation_j: &Quat<f64>,
        translation_j: &Vector3<f64>,
        ...
        // Compute relative rotation: delta_R = R_i^T * R_j
        let r_i_inv = rotation_i.conjugate();
        let delta_rotation = r_i_inv * *rotation_j;

        // Compute relative velocity in frame i:
        // delta_v = R_i^T * (v_j - v_i - g * dt)
        let vel_diff = velocity_j - velocity_i - gravity * delta_time;
        let delta_velocity = rotate_vector_by_quat(&r_i_inv, &vel_diff);

        // Compute relative position in frame i:
        // delta_p = R_i^T * (p_j - p_i - v_i * dt - 0.5 * g * dt²)
        let pos_diff = translation_j - translation_i
            - velocity_i * delta_time
            - 0.5 * gravity * delta_time * delta_time;
        let delta_position = rotate_vector_by_quat(&r_i_inv, &pos_diff);
    ...
    /// Get the information matrix (inverse of covariance)
    /// Returns None if covariance is singular
    pub fn information_matrix(&self) -> Option<Matrix9<f64>> {
        self.covariance.try_inverse()
    }

    /// Get the Cholesky factor of the information matrix (sqrt information)
    /// This is used for whitening residuals: r_whitened = L * r
    /// Returns None if covariance is not positive definite
    pub fn sqrt_information(&self) -> Option<Matrix9<f64>> {
        use nalgebra::Cholesky;
        // Information = Cov^-1, sqrt_info = chol(Information) = chol(Cov^-1)
        // For numerical stability, compute Cholesky of covariance then invert
        let chol = Cholesky::new(self.covariance)?;
        let l = chol.l();
        // If Cov = L * L^T, then Cov^-1 = L^-T * L^-1
        // sqrt(Cov^-1) = L^-T (upper triangular)
        // We want lower triangular, so we return L^-1
        l.try_inverse()
    }
}

/// Helper to rotate a nalgebra Vector3 by a Quat
fn rotate_vector_by_quat(q: &Quat<f64>, v: &Vector3<f64>) -> Vector3<f64> {
    let v_solver = SolverVec3::new(v.x, v.y, v.z);
    let rotated = q.rotate_vec(v_solver);
    Vector3::new(rotated.x, rotated.y, rotated.z)
}

/// 15x15 matrix type for covariance (rotation, velocity, position, gyro_bias, accel_bias)
pub type Matrix15<T> = SMatrix<T, 15, 15>;

/// Visually-informed IMU preintegration for GBA
///
/// Unlike `RelativePoseConstraint` which expresses motion relative to a specific frame,
/// this contains the pose-invariant preintegration along with visually-informed posteriors.
/// This allows GBA to maintain IMU dynamics when optimizing poses.
///
/// The key insight: the preintegration is in body frame and doesn't depend on world poses.
/// GBA can use the same IMU residual as VIO:
///   r = R_i^T * (world_motion) - (preintegrated_motion + bias_correction)
///
/// The posterior estimates provide tighter initialization than IMU-only, while preserving
/// the ability to adjust poses without breaking IMU consistency.
#[derive(Debug, Clone)]
pub struct VisuallyInformedPreintegration {
    /// The raw IMU preintegration (pose-invariant, in body frame)
    pub preintegration: PreintegratedImu,
    
    /// Posterior estimate of relative rotation (from VIO optimization)
    /// Used for initialization/warm start, not as a hard constraint
    pub delta_rotation_posterior: Quat<f64>,
    
    /// Posterior estimate of relative velocity (from VIO optimization)
    pub delta_velocity_posterior: Vector3<f64>,
    
    /// Posterior estimate of relative position (from VIO optimization)
    pub delta_position_posterior: Vector3<f64>,
    
    /// Posterior covariance (15x15: rotation(3), velocity(3), position(3), gyro_bias(3), accel_bias(3))
    /// Tighter than IMU-only because it includes visual information
    pub covariance_posterior: Matrix15<f64>,
    
    /// Posterior gyroscope bias estimate
    pub gyro_bias_posterior: Vector3<f64>,
    
    /// Posterior accelerometer bias estimate
    pub accel_bias_posterior: Vector3<f64>,
