//! SLAM System with dynamic observations for incremental tracking
//!
//! This variant of SlamSystem is designed for use with visual trackers where
//! observations are generated incrementally frame-by-frame, rather than being
//! pre-generated. Each message to the GBA thread includes the updated observation
//! set, ensuring GBA always has access to all observations seen so far.
//!
...
/// Message from LBA to GBA: a single new frame with updated observations
...
pub struct LbaToGbaMsg {
    ...
    /// Updated observations for all frames seen so far (includes this frame)
    pub frame_observations: Arc<Vec<Vec<StereoObservation>>>,
}
...
/// SLAM System managing GBA threads with dynamic observations
pub struct SlamSystemDynamic {
...
impl SlamSystemDynamic {
    ...
    pub fn new(stereo_camera: StereoCamera<f64>) -> Self {
        ...
        let gba_handle = thread::spawn(move || {
            gba_thread_loop(to_gba_rx, from_gba_tx, gba_camera);
        });
    ...
    /// Send the latest frame to GBA for optimization with updated observations
    ///
    /// # Arguments
    /// * `frame_index` - Index of the frame to send
    /// * `world` - Current world state (only the specified frame is extracted)
    /// * `frame_observations` - Updated observations for all frames seen so far
    pub fn send_to_gba(
        &self,
        frame_index: usize,
        world: &WorldState,
...
impl Drop for SlamSystemDynamic {
    fn drop(&mut self) {
...
/// GBA thread main loop with dynamic observations
fn gba_thread_loop(
    ...
    let mut frame_observations: Arc<Vec<Vec<StereoObservation>>> = Arc::new(Vec::new());
    ...
    loop {
        ...
        for msg in messages {
            // Update observations to the latest received
            frame_observations = msg.frame_observations;
        ...
        if let Some(world) = &mut gba_world {
            ...
            run_bundle_adjustment(
                ...
                &gba_graph,
                world,
                &frame_observations,  // Use observations from message
...
mod tests {
    ...
    #[test]
    fn test_slam_system_dynamic_creation() {
        let camera = StereoCamera::simple(500.0, 640.0, 480.0, 0.1);
        let _system = SlamSystemDynamic::new(camera);
